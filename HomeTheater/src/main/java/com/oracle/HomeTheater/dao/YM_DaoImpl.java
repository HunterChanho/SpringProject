...
public class YM_DaoImpl implements YM_Dao {
	...
	public Bbs noticeContents(Bbs bbs) {
		...
		try {
			...
			System.out.println("YM_DaoImpl noticeContents bbs.getBbs_no, getBbs_category()->"
											+bbs.getBbs_no()+","+bbs.getBbs_category());
			System.out.println("YM_DaoImpl noticeContents bbsContemts.getBbs_no , getBbs_category ,getBbs_hit->"
											+bbsContents.getBbs_no()+","+bbsContents.getBbs_category());
						
			try {  
				//조회수 증가 시퀀스
				System.out.println("YM_DaoImpl noticeContents YM_ClickTitleCntHitBbs start  bbs.getBbs_category(), getBbs_no(), getBbs_hit()->"
											+bbs.getBbs_category()+","+bbs.getBbs_no()+","+bbs.getBbs_hit());
				session.update("YM_ClickTitleCntHitBbs", bbs);
				System.out.println("YM_DaoImpl noticeContents YM_ClickTitleCntHitBbs finish  bbs.getBbs_category(), getBbs_no(), getBbs_hit()->"
						+bbs.getBbs_category()+","+bbs.getBbs_no()+","+bbs.getBbs_hit());
...
//YM_noticeWrite 글 내용 작성하기
	...
	public int noticeWrite(Bbs bbs) {
		...
		try {
			...
			System.out.println("YM_DaoImpl noticeWrite 세션후  getResult->"+result);
		} catch (Exception e) {
